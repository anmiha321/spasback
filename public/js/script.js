...
let sliders = document.querySelectorAll('.sWs');
...
if (document.querySelector('.now-search__slider')) {
  new Swiper('.now-search__slider', {
    observer: true,
    ...
    spaceBetween: 30,
    loop: true,
    loopAddinitionalSlides: 1,
    preloadImage: false,
    pagination: {
      el: '.now-search__pagination',
      clickable: true,
      type: 'bullets'
    },
    navigation: {
      nextEl: '.now-search__btn_next',
      prevEl: '.now-search__btn_prev'
    },
    breakpoints: {
      1212: {
        slidesPerView: 3
...
document.addEventListener('DOMContentLoaded', () => {
  'use strict'; //Скрытие header при scroll
...
window.onload = function () {
  'use strict'; //Определние поддержки webp-формата
  ...
  testWebP(support => support ? body.classList.add('webp') : body.classList.add('no-webp'));
  ...
  switch (path) {
    ...
    default: // const mqlg = window.matchMedia("screen and (max-width: 1212px)");
    // const mqlgMF = window.matchMedia("screen and (min-width: 1212px)");
    // const mqmd = window.matchMedia("screen and (max-width: 991.98)");
    // const mqsm = window.matchMedia("screen and (max-width: 991.98)");
    // const mqxs = window.matchMedia("screen and (max-width: 560px)");
    // mqlgMF.addListener(doAnyOnLargeScreenMF);
    // doAnyOnLargeScreenMF(mqlgMF);
    // function doAnyOnLargeScreenMF(e) {
    //     if(e.matches) {}
    // }
  ...
  document.addEventListener('click', function (e) {
    ...
    if (target.hasAttribute('data-enter')) {
      openModal('enter');
    } else if (target.hasAttribute('data-reg')) {
      openModal('reg');
  ...
  function offScroll() {
    const scrollY = docElem.scrollTop;
  ...
  } // markNavItem('.nav__list');
  //Анимация


  const aboutAnimItems = document.querySelectorAll('[data-anim]');

  if (aboutAnimItems.length > 0) {
    window.addEventListener('scroll', animOnScroll);

    function animOnScroll() {
      for (let index = 0; index < aboutAnimItems.length; index++) {
        const animItem = aboutAnimItems[index];
        ...
        if (scrollY > animItemOffset - animItemPoint && scrollY < animItemOffset + animItemHeight) {
          ...
          if (!animItem.classList.contains('_animNoHide')) {
...
$(function () {
  $('input[type="tel"]').mask('+7 (999) 999-9999'); //SEND MAIL

  function nameTest(input) {
    return /^[a-zA-Z]{2,}$|^[а-яёА-ЯЁ]{2,}$/g.test(input.value);
  }

  function emailTest(input) {
    return /^([a-z0-9_-]+\.)*[a-z0-9_-]+@[a-z0-9_-]+(\.[a-z0-9_-]+)*\.[a-z]{2,6}$/.test(input.val());
  }

  function sendQuestion(e, btn) {
    btn = $(this).find('input[type="submit"]');
    const msg = $(this).find('textarea[name="faq_msg"]');
    const email = $(this).find('input[name="faq_email"]');
    ...
    if (!emailTest(email)) {
      ...
      $.ajax({
        ...
        success: msg => {
          ...
          if (msg == '0') {
            err.addClass('active').html('Все поля должны быть заполнены');
            setTimeout(() => err.removeClass('active'), 5000);
          } else if (msg == '3') {
            err.addClass('active').html('Электронная почта указана некорректно');
            setTimeout(() => err.removeClass('active'), 5000);
      ...
      err.addClass('active').html('Сообщение пустое');
      setTimeout(() => err.removeClass('active'), 5000);
    }
  }
  ...
  function loadFile() {
    ...
    const waiting = $(this).parents('form').find('[data-waiting]');
    let file = $(this)[0].files[0];
    let fd = new FormData();
    fd.append('file', file);
    $.ajax({
      url: '/backend/file.php',
      type: 'POST',
      contentType: false,
      processData: false,
      data: fd,
      ...
      success: msg => {
        ...
        switch (msg) {
          case '000':
            err.addClass('active').html('Вы забыли создать каталог временных файлов на сервере!');
            setTimeout(() => err.removeClass('active'), 5000);
            $(this).val('');
            break;
          ...
          case '9':
            this.parentNode.insertAdjacentHTML('afterend', `
                            <p class="error active">
                                Допустимые расширения для файлов: ".jpeg",".jpg",".png",".tiff".
                            </p>
                            `);
            setTimeout(() => this.parentNode.nextElementSibling.classList.remove('active'), 5000);
            break;

          default:
            $(this).siblings('#photo-name').val(msg);
            document.querySelector('#label-photo').insertAdjacentHTML('afterend', `
                            <div id="photo-preview" class="search-req__preview-photo">
                                <img data-preview-del src="/css/img/svg/x.svg" alt="Закрыть" class="search-req__preview-close">
                                <a href="/uploads/tmp/${msg}" class="search-req__preview-link" target="_blank">
                                    <img src="/uploads/tmp/${msg}" class="search-req__preview-img" alt='preview'>
                                </a>
                            </div>
                            `);
        }
  ...
  function removeFile() {
    ...
    let fd = new FormData();
    fd.append('file_del', file_name_del);
    $.ajax({
      url: '/backend/file.php',
      type: 'POST',
      contentType: false,
      ...
      data: fd,
      success: msg => {
        $(this).parent().prev().find('#photo-missed').val('');
        $(this).parent().prev().find('#photo-name').val('');
  ...
  $(document).on('change', '[data-radio]', isChecked);

  function isChecked() {
    if ($(this).is(':checked')) {
      $('[data-radio]').parent().removeClass('checked');
      $(this).parent().addClass('checked');
    }
  }
  /* Маска ввода символов для input, только Кириллица и Латиница */


  if ($('[data-alpha]').length > 0) {
    $('[data-alpha]').on('keyup', function () {
      $(this).val(this.value.match(/^[a-zA-Zа-яА-ЯёЁ]*/)); //[ ]?[a-zA-Zа-яА-ЯёЁ]*$ - разрешает 1 пробел
    });
  }

  if ($('[data-alpha-fio]').length > 0) {
